...
struct ShotCameraView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                HStack {
                    ...
                    Spacer()
                    Text("Shot Tracer").font(.headline)
                    Spacer()
                ...
                Spacer()
                ZStack {
                    RoundedRectangle(cornerRadius: 24).stroke(.white.opacity(0.45), lineWidth: 2)
                    Path { path in
                        path.move(to: CGPoint(x: 70, y: 350))
                        path.addCurve(
                            to: CGPoint(x: 265, y: 80),
                            control1: CGPoint(x: 120, y: 190),
                            control2: CGPoint(x: 205, y: 90)
                        )
                    }
                ...
                .frame(height: 440)
                .padding(.horizontal)
                Text("Keep the phone fixed, leave sky in the flight direction and stay outside the club and ball path.")
                    .font(.subheadline)
                    .foregroundStyle(.white.opacity(0.85))
                    .multilineTextAlignment(.center)
                    .padding(.horizontal)
                Text("The trace is visualised—not measured carry, apex, spin or complete 3D flight.")
                    .font(.caption)
        ...
        .fullScreenCover(isPresented: $showPicker) {
            VideoPicker { url in
                if let url {
                    let filename = copyVideoIntoGallery(url)
                    let round = store.activeRound
                    let course = round.flatMap { current in store.courses.first { $0.id == current.courseID } }
                    store.addGalleryVideo(filename: filename, courseName: course?.name, hole: round?.holeNumber)
                }
                showPicker = false
                dismiss()
            }
    ...
    private func copyVideoIntoGallery(_ source: URL) -> String {
